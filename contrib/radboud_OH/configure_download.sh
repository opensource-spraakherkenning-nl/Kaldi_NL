...
if [ ! -e models/Lang_OH ]; then
    ...
    if [ ! -e kaldi_nl_model_radboud_OH.tar.gz ]; then
        wget https://applejack.science.ru.nl/downloads/kaldi_nl_radboud/kaldi_nl_model_radboud_OH.tar.gz || fatalerror "Unable to download asr_nl models from applejack.science.ru.nl!"
    fi
    tar -C models --strip-components 1 -xvzf kaldi_nl_model_radboud_OH.tar.gz || fatalerror "Failure during extraction of models"

    rm kaldi_nl_model_radboud_OH.tar.gz
